...
-(void)mouseDown:(NSEvent *)theEvent {
    ...
    NSLog(@"mouseDown: %@", NSStringFromPoint(location));
//    self.currentTouchValue = [NSNumber numberWithFloat:location.y];
...
- (void)mouseDragged:(NSEvent *)theEvent{
    // Move the paddle vertically if the new position is within
    // the parent frame. SKNode positions are centered.
    NSPoint touchPoint = [theEvent locationInNode:self];
    NSLog(@"mouseDragged: %@", NSStringFromPoint(touchPoint));

    NSPoint movePoint = CGPointMake(self.position.x, self.position.y + touchPoint.y);
    if ([self withinParentFrame:movePoint])
        self.position = movePoint;
