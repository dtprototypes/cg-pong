...
- (void)didMoveToView:(SKView *)view
{
    if (!self.contentCreated) {
        ...
        self.winningText = [SKLabelNode labelNodeWithFontNamed:@"Arial"];
        self.winningText.fontSize = 200;
        self.winningText.hidden = YES;
        [self addChild:self.winningText];
...
- (void)createSceneContents
{
    ...
    self.bgImage = [SKSpriteNode spriteNodeWithImageNamed:@"cglogo"];
    [self.bgImage setScale:0.4];
    self.bgImage.position = CGPointMake(self.size.width/2, self.size.height/2);
    ...
    [self performSelector:@selector(serveBallRightwards) withObject:nil afterDelay:2.0];
...
- (void)didChangeSize:(CGSize)oldSize
{
    ...
    if (self.contentCreated) {
        ...
        self.bgImage.position = CGPointMake(self.size.width/2, self.size.height/2);
        self.winningText.position = CGPointMake(self.size.width/2, self.size.height * .25);
...
- (void)update:(NSTimeInterval)currentTime
{
    if (!self.gameOver){
        ...
        if (leftScoreNode.count >= kWinningScore || rightScoreNode.count >= kWinningScore){
            
            self.gameOver = YES;
            NSLog(@"Game Over");
            
            if(leftScoreNode.count >= kWinningScore)
                self.winningText.text = @"PLAYER ONE WINS";
            else
                self.winningText.text = @"PLAYER TWO WINS";

            self.winningText.hidden = NO;
            BallNode *ball = (BallNode *) [self childNodeWithName:@"ball"];
            if (ball) {
                [ball removeFromParent];
            ...
            [self checkForLostBallInPlay];
        }
    }
    
}

- (void)checkForLostBallInPlay {
    BallNode *ball = (BallNode *) [self childNodeWithName:@"ball"];
    CGRect bounds = self.frame;
    bounds.size.width = bounds.size.width + 10;
    bounds.size.height = bounds.size.height + 10;

    if (ball && !CGRectContainsPoint(bounds, ball.position)) {
        NSLog(@"checkForLostBallInPlay");
        [self removeBall];
        BallNode *newBall = [[BallNode alloc] init];
        [self addChild:newBall];
        [newBall resetPosition];
        [newBall serveLeftwards];
    }
...
- (void)playWinningSound {
    SKAction *sound = [SKAction playSoundFileNamed:@"winner.wav" waitForCompletion:NO];
